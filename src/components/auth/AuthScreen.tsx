...
export function AuthScreen({ onSuccess }: { onSuccess: () => void }) {
  ...
  return <main className="safe-area-top safe-area-bottom relative flex min-h-dvh items-center justify-center overflow-hidden bg-[#07051a] px-4 py-6 text-slate-100 sm:py-8">
    ...
    <div className="relative grid w-full max-w-5xl overflow-hidden rounded-[1.5rem] border border-white/10 bg-slate-950 sm:rounded-[2rem] lg:grid-cols-[.9fr_1.1fr]">
      ...
      <section className="p-5 sm:p-10"><div className="mb-6 flex items-center gap-3 lg:hidden"><img src="/brand-emblem.png" alt="Sahtekar Kim?" className="h-11 w-11 rounded-xl" /><div><p className="font-bold">Sahtekar Kim?</p><p className="text-xs text-slate-500">Oyuna devam etmek için giriş yap</p></div></div><div className="mb-6 sm:mb-7"><p className="mb-2 text-xs font-bold uppercase tracking-[.22em] text-indigo-300">{registering ? 'Yeni oyuncu' : 'Tekrar hoş geldin'}</p><h1 className="text-3xl font-black">{registering ? 'Ekibine katıl' : 'Oyuna giriş yap'}</h1><p className="mt-2 text-sm text-slate-400">{registering ? 'Bir oyun adı seç, sahne senin olsun.' : 'İstatistiklerin ve başarıların seni bekliyor.'}</p></div>
